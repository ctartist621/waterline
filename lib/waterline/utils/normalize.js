...
var normalize = module.exports = {
  ...
  criteria: function(origCriteria) {
    ...
    if (!criteria.where && !criteria.joins && !criteria.join && !criteria.limit && !criteria.skip &&
      ...
      !criteria.groupBy && !criteria.min && !criteria.max && !criteria.select && !criteria.count) {
    ...
    if (hop(criteria, 'where') && criteria.where !== null && hop(criteria.where, 'count')) {
      criteria.count = _.clone(criteria.where.count);
      delete criteria.where.count;
    }
